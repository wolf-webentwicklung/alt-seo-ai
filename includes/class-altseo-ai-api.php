...
class AltSEO_AI_API {
	...
	public function update_keywords( $post_id, $force_update = false ) {
		...
		$content_api  = new AltSEO_AI_Content_API();
		...
		foreach ( $images as $image ) {
			...
			if ( ! isset( $alt_array_saved[ $filename ] ) ) {
				...
				if ( $generated_alt ) {
					...
					$enhanced_alt                 = $this->enhance_alt_text( $generated_alt, $page_keywords, $post_id );
		...
		if ( has_post_thumbnail( $post_id ) ) {
			...
			if ( $featured_image_src ) {
				...
				if ( $featured_alt ) {
					...
					$enhanced_featured_alt = $this->enhance_alt_text( $featured_alt, $page_keywords, $post_id );
	 ...
	 * @since 1.0.0
	 */
	public function enhance_alt_text( $current_alt, $page_keywords, $post_id = null ) {
		// Detect language from post content if post_id is provided.
		$detected_language = 'English'; // Default.
		if ( $post_id ) {
			$content_api  = new AltSEO_AI_Content_API();
		...
		$prompt  = "Combine the following two elements into a single, natural-sounding image alt text:\n\n";
		$prompt .= '1) Current image alt description: ' . $current_alt . "\n";
		$prompt .= '2) Page-specific keywords that must be included: ' . $page_keywords . "\n\n";
		$prompt .= "Requirements:\n";
		$prompt .= "- Create ONE fluent sentence that naturally integrates all given keywords\n";
		$prompt .= "- Must include the page-specific keywords provided\n";
		...
		$prompt .= "Example: If image description is 'Tree on a field' and page keywords are 'agriculture, farming', then result should be: 'A tree standing in an agricultural farming field.'\n\n";
